...
export function ListPageSkeleton({ cards = 6, variant = "grid", rows = 8 }) {
  if (variant === "table") {
    return (
      <div className="overflow-hidden rounded-2xl border border-edge bg-surface/80 shadow-soft backdrop-blur-md animate-pulse">
        <div className="border-b border-edge bg-surface-muted/80 px-4 py-3">
        ...
        <div className="divide-y divide-edge">
          {[...Array(rows)].map((_, i) => (
            <div key={i} className="flex items-center gap-4 px-4 py-4">
              ...
              <div className="hidden h-3 w-24 rounded bg-surface-muted sm:block" />
              <div className="h-8 w-8 rounded-xl bg-surface-muted" />
            </div>
          ))}
        </div>
      </div>
    ...
    return (
      <div className="space-y-3 animate-pulse p-1">
        {[...Array(5)].map((_, i) => (
          <div
            key={i}
            className="h-20 rounded-2xl border border-edge/60 bg-zinc-200/50"
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="grid grid-cols-1 gap-4 animate-pulse p-1 md:grid-cols-2 xl:grid-cols-3 md:gap-5">
      {[...Array(cards)].map((_, i) => (
        <div
          key={i}
          className="rounded-2xl border border-edge bg-surface/80 p-5 shadow-soft backdrop-blur-md"
        >
