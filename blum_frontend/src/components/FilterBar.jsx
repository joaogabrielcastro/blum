...
const normalizeBrand = (brand) => {
  ...
  const commissionValue =
    typeof brand?.commission_rate === "number"
      ...
      ? parseFloat(brand.commission_rate.value || brand.commission_rate.rate) || 0
      : 0;
...
const FilterBar = ({
  ...
  const displayedBrands = showAllBrands ? normalizedBrands : normalizedBrands.slice(0, 5);
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between">
        ...
        <div className="flex-1">
          <div className="flex justify-between items-center mb-1">
            <h2 className="font-semibold text-gray-700">Filtrar por Representada:</h2>
            {normalizedBrands.length > 5 && (
              <button onClick={() => setShowAllBrands(!showAllBrands)} className="text-xs text-blue-600 hover:text-blue-800">
                {showAllBrands ? "Mostrar menos" : `Ver todas (${normalizedBrands.length})`}
              </button>
            )}
          </div>
          <div className="flex flex-wrap gap-2 items-center">
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors duration-200 ${
                selectedBrand === "all" ? "bg-blue-600 text-white shadow-md" : "bg-gray-200 text-gray-700 hover:bg-gray-300"
              }`}
            >
              Todas
            </button>
            ...
            {normalizedBrands.length > 0 ? (
                ...
                <div key={brand.id} className="relative group">
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors duration-200 relative ${
                      selectedBrand === brand.displayName ? "bg-blue-600 text-white shadow-md" : "bg-gray-200 text-gray-700 hover:bg-gray-300"
                    }`}
                  >
                    {brand.displayName}
                    {/* ✅ BADGE DE COMISSÃO - VISÍVEL PARA TODOS */}
                    <span className="absolute -top-2 -right-2 bg-purple-600 text-white text-xs px-2 py-1 rounded-full">{brand.commission}%</span>
                  </button>

                  {/* ✅ BOTÕES DE AÇÃO - APENAS ADMIN */}
                  {isAdmin && (
                    <div className="absolute -right-2 -top-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={(e) => handleEditClick(brand, e)} className="bg-blue-500 text-white rounded-full p-1 shadow-md hover:bg-blue-600" title="Editar Representada">
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          onDeleteBrand("brand", brand.displayName, brand.displayName);
                        }}
                        className="bg-red-500 text-white rounded-full p-1 shadow-md hover:bg-red-600"
                        title="Excluir Representada"
                      >
                        {confirmDelete === brand.displayName && deleteType === "brand" ? <span className="text-xs font-bold">✓</span> : <span className="text-xs">✕</span>}
                      </button>
                    </div>
                  )}
                  ...
                  {isAdmin && editingBrand === brand.id && (
                    <div className="absolute top-full left-0 mt-2 p-3 bg-white border border-gray-200 rounded-lg shadow-lg z-10 w-64">
                      <h4 className="font-semibold text-sm mb-2">Editar Comissão - {brand.displayName}</h4>
                      <div className="flex items-center gap-2 mb-2">
                        <input type="text" value={editCommission} onChange={handleCommissionChange} className="flex-1 px-2 py-1 border border-gray-300 rounded text-sm" placeholder="0.00" />
                        <span className="text-gray-600">%</span>
                      </div>
                      <div className="flex gap-2 justify-end">
                        <button onClick={handleCancelEdit} className="px-2 py-1 text-xs text-gray-600 hover:text-gray-800">Cancelar</button>
                        <button onClick={(e) => handleSaveEdit(brand.id, e)} className="px-2 py-1 text-xs bg-purple-600 text-white rounded hover:bg-purple-700">Salvar</button>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <span className="text-gray-500 text-sm ml-2">Nenhuma Representada cadastrada</span>
            )}
          </div>
          ...
          {isAdmin && confirmDelete && deleteType === "brand" && (
            <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-md text-sm">
              <p className="text-yellow-800 font-medium">Confirmar exclusão da Representada "{confirmDelete}"?</p>
              <p className="text-yellow-700 text-xs mt-1">Todos os produtos desta Representada também serão excluídos.</p>
              <div className="flex space-x-2 mt-2">
                <button onClick={onConfirmDelete} className="bg-red-600 text-white px-3 py-1 rounded text-sm hover:bg-red-700 transition-colors">Confirmar Exclusão</button>
                <button onClick={onCancelDelete} className="bg-gray-500 text-white px-3 py-1 rounded text-sm hover:bg-gray-600 transition-colors">Cancelar</button>
              </div>
            </div>
          )}
        </div>
