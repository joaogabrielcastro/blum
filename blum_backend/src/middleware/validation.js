...
exports.validateProduct = [
  ...
  body('minstock')
    .optional()
    .isInt({ min: 0 }).withMessage('Estoque mínimo deve ser um número inteiro positivo'),
...
exports.validateClient = [
  ...
  body('cnpj')
    ...
    .withMessage('CNPJ inválido - deve ter 14 dígitos ou estar formatado corretamente'),
...
exports.validateOrder = [
  ...
  body('items')
    .isArray({ min: 1 }).withMessage('Pedido deve conter pelo menos um item'),
  ...
  body('items.*.brand')
    .trim()
    .notEmpty().withMessage('Marca do produto é obrigatória'),
  ...
  body('discount')
    .optional()
    .isFloat({ min: 0, max: 100 }).withMessage('Desconto deve estar entre 0 e 100'),
...
exports.validateBrand = [
  body('name')
    .trim()
    .notEmpty().withMessage('Nome da marca é obrigatório')
    .isLength({ min: 2, max: 255 }).withMessage('Nome deve ter entre 2 e 255 caracteres'),
  
  body('commission_rate')
    .optional()
    .isFloat({ min: 0, max: 100 }).withMessage('Taxa de comissão deve estar entre 0 e 100'),
...
exports.validateUserCreate = [
  body('username')
    ...
    .matches(/^[a-zA-Z0-9_]+$/).withMessage('Usuário pode conter apenas letras, números e underscore'),
  ...
  body('role')
    .isIn(['admin', 'salesperson']).withMessage('Role deve ser admin ou salesperson'),
...
exports.validateSearchQuery = [
  query('q')
    .optional()
    .trim()
    .isLength({ min: 1 }).withMessage('Termo de busca não pode ser vazio'),
