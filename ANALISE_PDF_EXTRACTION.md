...
function extractElgin(fullText) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const elginPattern = /(\d{6,8})\s+([A-Za-zÀ-ÿ0-9\s\-\/]{10,100}?)\s+(\d{1,5})\s+([\d,.]+)/;
    ...
    if (match) {
      ...
      const unitPrice = parseFloat(priceRaw.replace(/\./g, "").replace(",", "."));
      ...
      if (quantity > 0 && unitPrice > 0 && !itemsMap.has(code)) {
        ...
        console.log(`   ✅ ELGIN: ${code} - ${description.substring(0, 30)}... | Qtd: ${quantity} | R$ ${unitPrice}`);
      }
    }
  }
